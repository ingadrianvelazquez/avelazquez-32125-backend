...
import { configGraphqlProduct, configGraphqlCart } from "./models/GraphQL.js";
...
app.use("/api/products", graphqlHTTP(configGraphqlProduct));
app.use("/api/randoms", randomRouter);
//app.use('/api/cart', cartRouter)
app.use("/api/cart", graphqlHTTP(configGraphqlCart));
...
if (MODE == "CLUSTER" && cluster.isPrimary) {
  ...
  for (let aux = 0; aux < numCPUs; aux++) cluster.fork();
  ...
  httpServer.listen(PORT, () =>
    loggerConsole.info(`Server UP on ${PORT} with processID: ${process.pid}`)
  );
